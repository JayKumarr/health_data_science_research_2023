...
```{r} 
filtered.data <- raw.data %>% dplyr::filter(medical_specialty %in% c("Neurology", "Radiology", "Neurosurgery")) 
```
...
analysis.data <- filtered.data %>%
    ...
    left_join(select(analysis.data, -transcription), by="note_id")
```
...
```{r}
tokenized.data <- analysis.data.sw.removed %>% tidytext::unnest_tokens(ngram, transcription, token = "ngrams", n=2, to_lower = TRUE)
```
...
```{r}
tokenized.data2 %>% dplyr::group_by(medical_specialty) %>% dplyr::distinct(ngram) %>% dplyr::summarise(n=dplyr::n())
```

Let's plot some distribution of words
